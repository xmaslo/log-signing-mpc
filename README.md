...
To run tests, follow:
1. `pip install -r .\tests\requirements.txt`
2. In the `tests\common`, modify the `IS_DOCKER` to be **False**.
3. In the `scripts`, to run servers: `.\run_servers_on_localhost.ps1 ..\target\release`.
4. `pytest`.
5. Make sure the keys were not already generated by the previous test runs, otherwise first test will fail (if generated, keys are usually present in the `target\release` under names `local-shareX`, where X is the number of a share).
